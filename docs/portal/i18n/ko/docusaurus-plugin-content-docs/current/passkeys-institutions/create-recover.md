...
5. Pali가 일치하는 account를 balance 및 optional activity hint와 함께 표시합니다.
6. Pali가 선택된 account를 local wallet state로 다시 import합니다.

Settings recovery는 deploy된 account를 discover하고, 이미 local에 있는 account를 건너뛰며, 사용자가 import할 matching account를 선택하게 합니다.

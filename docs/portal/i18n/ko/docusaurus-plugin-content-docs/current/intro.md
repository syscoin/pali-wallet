...
Pali는 기관 및 고급 dapp을 위한 Pali smart account도 지원합니다. dapp은 Pali에 WebAuthn-backed smart account 생성 및 deployment, 생성 중 smart-account module policy 연결, 이후 `wallet_sendCalls`를 통한 atomic batch 실행을 요청할 수 있습니다. 기존 smart account recovery는 Pali에서 처리됩니다.

Pali가 다른 점은 이 조합입니다. 하나의 확장 프로그램에서 EVM과 Syscoin UTXO 흐름을 모두 다루고, Multicall3와 RPC batch fallback으로 빠르게 읽으며, 일반 계정과 modular smart account를 함께 사용할 수 있습니다. dapp에는 MetaMask 호환 표준을 제공하고, 서명, 복구, account policy는 Pali 서버가 아니라 사용자 승인과 on-chain module로 강제되는 self-custody 보안 모델을 따릅니다. 전체 그림은 [Pali란 무엇인가요?](./start-here/what-is-pali.md)를 참고하세요.

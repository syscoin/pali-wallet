...
Pali v4는 속도와 유연한 서명 권한을 중심으로 지갑을 재구축했습니다. EVM과 UTXO 네트워크 모두에서 RPC를 batch 처리하여 훨씬 빠른 UI를 제공하고, smart account는 ERC-7579 / ERC-4337 표준을 따르며, validator(서명할 수 있는 역할)와 guardian(복구할 수 있는 역할)이 동일한 안정적인 account address 위에서 서로 독립적이고 업그레이드 가능한 역할로 동작하는 modular 인가 모델을 채택합니다. 전체 그림은 [Pali란 무엇인가요?](./start-here/what-is-pali.md)를 참고하세요.

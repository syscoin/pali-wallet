...
| `wallet_createPasskeyAccount` | EVM | Créer ou récupérer un compte intelligent passkey. | Oui |
| `wallet_sendCalls` | EVM | Envoyer une requête groupée EIP-5792. | Oui |
| `wallet_getCapabilities` | EVM | Lire les capacités du compte. | Non |
| `wallet_getCallsStatus` | EVM | Stub de compatibilité ; erreur sur les ids inconnus. | Non |
| `wallet_showCallsStatus` | EVM | Stub de compatibilité ; retourne `null`. | Non |

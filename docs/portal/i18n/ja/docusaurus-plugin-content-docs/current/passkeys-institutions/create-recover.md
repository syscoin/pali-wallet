...
5. Paliは一致するアカウントを残高と任意のアクティビティヒント付きで表示します。
6. Paliは選択されたアカウントをローカルウォレット状態へインポートします。

設定からの復元はデプロイ済みアカウントを検出し、ローカルにすでに存在するアカウントをスキップし、インポートする一致アカウントをユーザーが選択できるようにします。

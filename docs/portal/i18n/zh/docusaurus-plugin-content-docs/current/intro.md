...
Pali 的不同之处在于这些能力的组合：一个扩展同时支持 EVM 和 Syscoin UTXO 流程；通过 Multicall3 和 RPC 批量 fallback 提供快速读取；同时支持普通账户和模块化智能账户；为 dapp 提供 MetaMask 兼容标准；并采用自托管安全模型，签名、恢复和账户策略由用户批准与链上模块执行，而不是由 Pali 服务器执行。完整介绍见 [什么是 Pali？](./start-here/what-is-pali.md)。

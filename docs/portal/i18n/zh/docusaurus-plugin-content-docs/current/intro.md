...
Pali 还为机构和高级 dapp 支持 Passkey 智能账户。dapp 可以请求 Pali 创建并部署由 WebAuthn 支持的智能账户，在创建期间附加 sponsor 策略，并随后通过 `wallet_sendCalls` 执行原子批量操作。现有 Passkey 账户的恢复在 Pali 中处理。

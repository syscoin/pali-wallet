...
5. 如果 required sponsor policy 需要 sponsor URL 元数据，则发出警告。

## dapp 创建/恢复的幂等性

当 dapp 调用 `wallet_createPasskeyAccount` 时，Pali 会先检查是否存在与请求的 sponsor 策略匹配的现有链上 Passkey 账户。如果匹配账户已在本地存在，Pali 会复用它，而不是创建重复账户。如果它存在于链上但不在本地，Pali 可以将其恢复到钱包中。

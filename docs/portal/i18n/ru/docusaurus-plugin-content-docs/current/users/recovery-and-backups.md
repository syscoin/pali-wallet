...
Cloud-synced passkeys все еще безопасны для обычного использования: приватный ключ не передается Pali или dapp, WebAuthn остается origin-bound, а user verification все еще выполняется platform authenticator. Компромисс в том, что platform account становится частью вашей модели безопасности кошелька. Для cold storage, treasury funds или больших долгосрочных балансов предпочитайте device-bound authenticator или hardware security key и держите в синхронизированных smart accounts только меньшие операционные средства.
...
## Восстановление smart accounts

Восстановление Pali passkey привязано к credential и использует on-chain account discovery. Любая установка Pali, которая может получить WebAuthn assertion от того же passkey credential, может импортировать соответствующие deployed smart accounts. Recovery flow:
...
5. Добавляет recoverable accounts в кошелек.

## Идемпотентность create/recover для dapp

Когда dapp вызывает `wallet_prepareSmartAccount`, Pali сначала проверяет, соответствует ли существующий on-chain smart account запрошенной smart-account module policy. Если matching account уже существует локально, Pali повторно использует его вместо создания дубликата. Если он существует on-chain, но не локально, Pali может восстановить его в кошелек.

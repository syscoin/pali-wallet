...
Восстановление Pali passkey привязано к credential и использует on-chain account discovery. Любая установка Pali, которая может получить WebAuthn assertion от того же passkey credential, может импортировать соответствующие deployed passkey accounts. Recovery flow:

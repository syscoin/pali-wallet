...
export const AccountHeader: FC<IAccountHeader> = ({ importSeed }) => {
  ...
  const AccountMenu = () => {
    return (
      <Menu as="div" className="absolute right-2 inline-block text-right">
        <Menu.Button className="inline-flex justify-center w-full  text-sm font-medium text-white rounded-full hover:bg-opacity-30 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75 ">
          {encriptedMnemonic && !importSeed && (
            <Icon
              name="dots"
              className="text-brand-white z-0"
              id="kebab-menu-btn"
            />
          )}
        </Menu.Button>
        ...
        >
          ...
          >
            <h2 className="bg-menu-primary pt-8 pb-6 text-brand-white border-b border-dashed border-dashed-light w-full text-center mb-3">
              ACCOUNT SETTINGS
            </h2>
            ...
            <Menu.Item>
              <Disclosure>
                {({ open }) => (
                  <>
                    <Disclosure.Button className="flex py-3 justify-start items-center w-full text-base px-5 cursor-pointer transition-all duration-200 hover:bg-bkg-3 accounts-btn">
                      <Icon
                        name="user"
                    ...
                    <Disclosure.Panel className="scrollbar-styled pb-2 text-sm bg-menu-secondary h-40 overflow-auto">
                      <li
                        onClick={() => history.push('/account-newaccount')}
                        className="flex items-center p-2.5 text-sm font-medium text-brand-white transition transform bg-menu-secondary mb-4 backface-visibility-hidden active:bg-opacity-40 focus:outline-none justify-center duration-300 mx-auto w-full border-b border-dashed border-menu-dasheddark cursor-pointer hover:bg-bkg-2 create-account-btn"
                      >
                        <Icon
                          name="appstoreadd"
            ...
            <Menu.Item>
              ...
              >
                <Icon
                  name="key"
                  className="text-brand-white ml-1 mr-4 lock-btn"
                />
  ...
  return (
    <div className="flex items-center justify-between bg-bkg-3 p-1">
      <div className="flex items-center w-full text-brand-white">
        ...
        <IconButton type="primary" shape="circle" className="mt-3">

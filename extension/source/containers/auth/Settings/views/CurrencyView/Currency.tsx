...
import { useController, usePrice, useStore, useFormat, useUtils, useAccount } from 'hooks/index';
...
const CurrencyView = () => {
  ...
  const { accounts, activeAccountId, fiat, activeNetwork } =
    useStore();

  const [selectedCoin, setSelectedCoin] = useState(String(fiat.current));
  const [checkValueCoin, setCheckValueCoin] = useState('usd'); const [confirmed, setConfirmed] = useState(false);
    ...
    <AuthViewLayout title="FIAT CURRENCY">
      {confirmed && (
        <Modal
          ...
          description={`Now you will see the values in your wallet in SYS and ${selectedCoin.toUpperCase() || 'USD'}`}
        />
      )}
      ...
      <div className="flex flex-col justify-center items-center">
        ...
        >
          ...
          >
            {getSymbolFromCurrency(selectedCoin ? selectedCoin.toUpperCase() : fiat.current ? String(fiat.current).toUpperCase() : 'USD')
            }

            <p className="ml-2">
              {selectedCoin ? selectedCoin.toUpperCase() : fiat.current ? String(fiat.current).toUpperCase() : 'USD'}
            </p>
          ...
          >
            {fiat && fiat.availableCoins && (
              <Menu.Items
                className="scrollbar-styled rounded-xl overflow-auto h-44 bg-bkg-4 border border-fields-input-border text-brand-white w-full font-poppins shadow-2xl absolute z-10 mt-2 py-3 origin-top-right"
              >
        ...
        <div className="text-center flex justify-center flex-col items-center">
          {activeNetwork == 'testnet' ? (
            ...
            <>
              ...
              <p>{getFiatAmount(activeAccount?.balance || 0, 4, String(selectedCoin ? selectedCoin : fiat.current ? fiat.current : 'USD'))}</p>
            </>
          )}
        </div>
        ...
        <div className="relative text-sm font-medium text-brand-royalblue ">
          ...
          <div className="absolute right-4 bottom-1.5 flex justify-center items-center gap-x-3">
            ...
            <div className="flex justify-center items-center gap-x-3 border-l border-dashed border-gray-700">
              <Icon name="dolar" wrapperClassname="w-2 ml-4 mb-1.5" className="text-brand-royalblue" />
          ...
          <div className="absolute right-2 bottom-2 flex justify-center items-center gap-x-3">
            ...
            >
              ...
              >
                {fiat && fiat.availableCoins && (
                  <Menu.Items
                    className="scrollbar-styled rounded-xl overflow-auto h-56 bg-bkg-4 border border-fields-input-border text-brand-white w-44 font-poppins shadow-2xl absolute z-10 right-0 bottom-10 mt-2 py-3 origin-bottom-right"
                  >
                    {Object.entries(fiat.availableCoins).map(([key]) => {
                      return (
                        <Menu.Item>

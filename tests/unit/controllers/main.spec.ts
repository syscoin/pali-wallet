...
describe('wallet creation tests', () => {
  ...
  it('should throw an error if switch network fails', async () => {
    ...
    const { activeNetwork, activeAccount, isPendingBalances, error } =
      store.getState().vault;
  ...
  it('should set an utxo network as the active network and update the active account', async () => {
    ...
    //* from the above test after switching network to polygon
    expect(currentActiveNetwork.chainId).toBe(networks.ethereum[137].chainId);
